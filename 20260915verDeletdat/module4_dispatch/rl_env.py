...
class TaxiRepositionEnv(gym.Env):
    ...
    def __init__(
        ...
        incentive_weight: float = 0.0,  # Surge Pricing 대체: 핫스팟 분포 일치도 보상 가중치 (탭4 슬라이더)
    ):
        super().__init__()
    ...
    def _distribution_match_score(self, idle: dict) -> float:
        """
        [Surge Pricing 대체] "예측수요 비율대로 유휴택시가 얼마나 잘 퍼져있는지" 0~1 점수.
        1.0 = 완벽히 비례 배분, 0.0 = 완전히 어긋남 (Total Variation Distance 기반).
        예측수요가 전부 0이면(모델 없음/워밍업 등) 판단 불가하므로 0.0(가산 없음) 반환.
        """
        predicted = self._predicted_demand()
        total_demand = sum(predicted.values())
        total_idle = sum(idle.values())
        if total_demand <= 0 or total_idle <= 0:
            return 0.0

        diff_sum = 0.0
        for category in ZONE_CATEGORIES:
            target_ratio = predicted.get(category, 0.0) / total_demand
            actual_ratio = idle.get(category, 0) / total_idle
            diff_sum += abs(target_ratio - actual_ratio)

        return max(0.0, 1.0 - diff_sum / 2.0)  # TVD는 0~1 범위라 1에서 빼면 바로 점수가 됨
