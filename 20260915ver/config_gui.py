...
"Module 4 — 배차/인센티브":[("incentive_weight","인센티브 리워드 가중치",0,5,2,0),("mock_available_taxis_min","테스트 가용택시 최소",0,500,0,1),("mock_available_taxis_max","테스트 가용택시 최대",0,500,0,1),("mock_taxi_count","배차 테스트 택시 수",1,500,0,1),("mock_passenger_count","배차 테스트 승객 수",1,500,0,1)],
...
class ConfigGUI:
 ...
 def run_training(self):
    ...
    try:
        # 0. 지도/시뮬레이션 환경 먼저 생성 (지역·설정 바뀐 채로 학습만 누르면
        #    옛 지도로 그대로 학습되는 문제 방지 - run_main()과 동일한 단계)
        self.status.config(text="0/3 지도(build_env.py) 생성 중...")
        self.root.update()

        self._run(os.path.join("module1_simulation", "build_env.py"), blocking=True)
