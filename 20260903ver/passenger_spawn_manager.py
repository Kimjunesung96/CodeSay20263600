...
class PassengerSpawnManager:
    def __init__(self, zones: dict, sim_start_hour: float,
                 school_pop_base: int = 400, company_pop_base: int = 100,
                 seed: int = None):
        if seed is not None:
            random.seed(seed)

        self.sim_start_hour = sim_start_hour
        self.school_pop_base = school_pop_base
        self.company_pop_base = company_pop_base
    ...
    def _process_arrivals(self, timeout_removed_pids: set, now_seconds: float):
        ...
        for pid in disappeared:
            ...
            if category == "restaurant" and not was_lunch:
                self._restaurant_pending.append((now_seconds + 3600.0, None))
    ...
    def _maintain_morning(self, now_seconds: float, now_hour: float):
        ...
        if 7.0 <= now_hour < 8.0:
            ...
            if random.random() < rate:
                taxi_prob = (now_hour - 7.0) / 1.0  # 7시=0, 8시=1로 선형 증가
        ...
        if 8.0 <= now_hour < 10.0:
            rate = self.company_pop_base / 7200.0  # 윈도우 2시간
            if random.random() < rate:
                taxi_prob = (now_hour - 8.0) / 2.0  # 8시=0, 10시=1로 선형 증가
                if random.random() < taxi_prob:
                    start_e = self._pick_edge(self.transit_edges)
    ...
    def _maintain_lunch(self, now_seconds: float, now_hour: float):
        if not self._lunch_12_done and now_hour >= 12.0:
            self._lunch_12_done = True
            release_total = int(self.absorbed.get("company", 0) * 0.8)
            self._lunch_release_taxi_count = int(release_total * 0.10)
        ...
        if not self._lunch_13_done and now_hour >= 13.0:
    ...
    def _maintain_school_afternoon(self, now_seconds: float, now_hour: float):
        if self._school_afternoon_pool_snapshot is None and now_hour >= 16.0:
            self._school_afternoon_pool_snapshot = int(self.absorbed.get("school", 0) * 0.01)

        if self._school_afternoon_pool_snapshot and 16.0 <= now_hour < 17.0:
    ...
    def _maintain_evening(self, now_seconds: float, now_hour: float):
        if self._evening_pool_snapshot is None and now_hour >= 18.0:
            self._evening_pool_snapshot = self.absorbed.get("company", 0)

        if self._evening_pool_snapshot is None:
            return

        if 18.0 <= now_hour < 22.0:
            rate = self._evening_pool_snapshot / (4 * 3600.0)
            if random.random() < rate:
                if random.random() < 0.30:  # 버스35%+지하철35% 제외 -> 30%만 택시
                    dest_pool = self.residential_edges if random.random() < (25 / 30) else self.restaurant_edges
                    start_e = self._pick_edge(self.company_edges)
        ...
        elif 23.0 <= now_hour < 24.0 and not self._evening_dump_done:
