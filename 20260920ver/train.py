...
# [팀 병합] 팀원(Fetex-ldk) 버전에서 쓰던 축적 로그. run_and_measure()가 시뮬레이션마다
# append하는 기존 방식이라 이미 몇 달치(수만 건)가 쌓여 있음 — data/sim_logs가 아직 비어있는
# 초기 상태에서도 학습이 가능하도록 두 로그 소스를 함께 사용.
...
def prepare_real_sequence_dataset(max_lag=None, log_path=None):
    ...
    logs = [log_path] if log_path else [DEMAND_LOG_GLOB, LEGACY_DEMAND_LOG_GLOB]

    if not log_path:
        found_paths = sorted(glob.glob(DEMAND_LOG_GLOB)) + sorted(glob.glob(LEGACY_DEMAND_LOG_GLOB))
        if not found_paths:
            raise FileNotFoundError(
                f"{DEMAND_LOG_DIR} 또는 {LEGACY_DEMAND_LOG_DIR}에 수요 로그가 없습니다. "
