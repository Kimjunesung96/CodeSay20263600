...
def map_pois_to_edges(pois: dict, net, search_radius: float = 80.0) -> dict:
    ...
    total_coords = sum(len(v) for v in pois.values())
    ...
    # D-3: 좌표 변환이 전량 실패하면 카테고리가 전부 비어 조용히 랜덤 배정으로 넘어간다.
    # pyproj 미설치가 가장 흔한 원인이므로 여기서 명시적으로 경고한다.
    if total_coords and conversion_failures == total_coords:
        print(
            f"[경고] POI 좌표 변환이 {total_coords}건 전부 실패했습니다 (pyproj 미설치 가능성). "
            f"구역이 실제 지도와 무관하게 임의 배정될 수 있습니다 — "
            f"requirements.txt의 pyproj/geopandas 설치를 확인하세요."
        )
