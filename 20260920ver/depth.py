...
class ProjectAnalyzerApp:
    ...
    def build_ui(self):
        ...
        # 체크된 파일들의 내용을 하나의 클립보드 텍스트로 복사
        tk.Button(
            self.bottom_frame,
            text="📋 선택된 파일 내용 복사",
            command=self.copy_selected_contents,
            bg="lightgreen",
            font=("Arial", 11, "bold")
        ).pack(fill="x", pady=(0, 5), side="bottom", ipady=8)
    ...
    def copy_selected_contents(self):
        """체크된 여러 파일의 내용을 파일별 구분선과 함께 하나의 텍스트로 복사."""
        selected_paths = [
            path for path in self.filtered_files
            if self.file_vars.get(path) and self.file_vars[path].get()
        ]
        ...
        if not selected_paths:
            messagebox.showwarning("경고", "내용을 복사할 파일을 체크해주세요.")
            return
        ...
        try:
            ...
            self.status_var.set(
                f"총 {len(selected_paths)}개 파일의 내용을 클립보드에 복사했습니다. "
                f"Ctrl+V로 한 번에 붙여넣을 수 있습니다."
            )
