...
TABS = {
    "Module 1 — 맵/시뮬레이션": [
        ...
        ("num_passengers", "탑승자 수", 0, 1000, 200, 0, True),
        ("sim_start_hour", "시뮬레이션 시작 시각 (시)", 0, 23, 5, 0, True),
        ("sim_end_hour", "시뮬레이션 종료 시각 (시)", 1, 24, 24, 0, True),
        ("passenger_wait_timeout", "승객 최대 대기시간 (초, 미배차시 소멸)", 60, 3600, 900, 0, True),
    ],
    "Module 2 — 전처리": [
...
class ConfigGUI:
    def __init__(self, root):
        ...
        for tab_name, params in TABS.items():
            ...
            if tab_name == "Module 1 — 맵/시뮬레이션":
                row = ttk.Frame(tab_frame)
                row.pack(fill="x", pady=(0, 8))
                ttk.Label(row, text="지역 검색 (위경도·기온 자동설정)", width=24).pack(side="left")
                self.region_search = RegionSearchEntry(row, default=DEFAULT_REGION, width=18)
                self.region_search.pack(side="right")

                map_row = ttk.Frame(tab_frame)
                map_row.pack(fill="x", pady=(0, 8))
                self.use_real_map_var = tk.BooleanVar(value=False)
                ttk.Checkbutton(
                    map_row, text="실제 지도(OSM)로 도로망 생성 — 끄면 합성 격자 그리드(과제 필수 최소구성) 사용",
                    variable=self.use_real_map_var
                ).pack(side="left")

                strategy_row = ttk.Frame(tab_frame)
                strategy_row.pack(fill="x", pady=(0, 8))
                ttk.Label(strategy_row, text="택시 전략 (patrol=무작위순찰 / prepositioned=붐빔예측이동)", width=24).pack(side="left")
                self.taxi_strategy_var = tk.StringVar(value="patrol")
                ttk.Combobox(strategy_row, textvariable=self.taxi_strategy_var,
                             values=["patrol", "prepositioned"], state="readonly", width=15).pack(side="right")

                seed_row = ttk.Frame(tab_frame)
                seed_row.pack(fill="x", pady=(0, 8))
                ttk.Label(seed_row, text="승객 시드 (비우면 매번 랜덤, A/B비교 시 숫자 고정)", width=24).pack(side="left")
                self.passenger_seed_var = tk.StringVar(value="")
                ttk.Entry(seed_row, textvariable=self.passenger_seed_var, width=15).pack(side="right")
        ...
        headless_btn = tk.Button(
            run_frame, text="🧪 GUI 없이 테스트 (measure_wait_time.py compare)",
            command=self.run_headless_compare, bg="#1565c0", fg="white",
            font=("맑은 고딕", 10, "bold"), height=2
        )
        headless_btn.pack(fill="x", pady=(6, 0))
    ...
    def get_current_config(self):
        region = self.region_search.get() or DEFAULT_REGION
        config = {
            "freq": self.freq_var.get(),
            "region": region,
            "use_real_map": self.use_real_map_var.get(),
            "taxi_strategy": self.taxi_strategy_var.get(),
        }
        # 승객 시드: 빈 문자열이면 None(=매번 랜덤), 숫자면 int로 고정
        seed_text = self.passenger_seed_var.get().strip()
        config["passenger_seed"] = int(seed_text) if seed_text.isdigit() else None
    ...
    def set_config(self, config):
        ...
        if "region" in config:
            self.region_search.set(config["region"])
        if "use_real_map" in config:
            self.use_real_map_var.set(bool(config["use_real_map"]))
        if "taxi_strategy" in config:
            self.taxi_strategy_var.set(config["taxi_strategy"])
        if "passenger_seed" in config:
            seed_val = config["passenger_seed"]
            self.passenger_seed_var.set("" if seed_val is None else str(seed_val))
        for key, (var, decimals, is_int) in self.vars.items():
            if key in config:
                val = config[key]
    ...
    @staticmethod
    def _utf8_env():
        """자식 파이썬 프로세스가 Windows 기본 코드페이지(cp949) 대신 UTF-8로 출력하게 강제.
        이걸 안 하면 build_env.py/main.py/train.py 콘솔 출력의 한글이 깨져 보입니다."""
        env = os.environ.copy()
    ...
    def _run_script(self, script_name, label, cwd=None):
        ...
        try:
            if os.name == "nt":
                ...
                subprocess.Popen([sys.executable, script_path], cwd=cwd or ROOT, env=self._utf8_env())
            return True
    ...
    def run_headless_compare(self):
        """GUI 없이 테스트 버튼: config 저장 → measure_wait_time.py compare 실행
        (SUMO GUI 창을 띄우지 않고 headless로 patrol/prepositioned 두 전략을 자동 비교).
        measure_wait_time.py가 내부적으로 build_env.py를 전략별로 다시 호출하므로
        여기서 build_env.py를 따로 먼저 실행할 필요는 없습니다."""
        self.save_config_only()
        self.status.config(text="GUI 없이 테스트 실행 요청 완료 — 새 콘솔 창에서 patrol vs prepositioned 비교 결과 확인하세요 (시간이 걸릴 수 있습니다)")
        self.root.update()
        self._run_script_with_args("measure_wait_time.py", ["compare"], "GUI 없이 테스트(compare)")

    def _run_script_with_args(self, script_name, args, label):
        script_path = os.path.join(ROOT, script_name)
        try:
            if os.name == "nt":
                subprocess.Popen(
                ...
                subprocess.Popen([sys.executable, script_path, *args], cwd=ROOT, env=self._utf8_env())
            return True
        except Exception as e:
            messagebox.showerror("실행 오류", f"{script_name} 실행 실패: {e}")
            return False
