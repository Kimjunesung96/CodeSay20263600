...
def lookup_region(place_name: str, margin_deg: float = 0.006) -> dict:
    """
    place_name(지역명)을 Nominatim으로 검색해서 위경도 범위를 반환.
    margin_deg: 검색된 중심점 기준으로 얼마나 넓은 사각형을 잡을지 (기본 약 600~700m 반경)
    ...
    if place_name in cache:
        print(f"[캐시 사용] '{place_name}' → {cache[place_name]['display_name']}")
    ...
    region = {
        "lat_min": round(lat - margin_deg, 6),
        "lat_max": round(lat + margin_deg, 6),
        "lng_min": round(lon - margin_deg, 6),
        "lng_max": round(lon + margin_deg, 6),
        "display_name": result.get("display_name", place_name),
    }

    cache[place_name] = region
    save_cache(cache)
